...
#for i in range(Num,Num+250):#part
for i in range(0,Num):#part    
    #print(i)
    ...
    lamt = yd['YA2'][i]/np.cos(tht)
    sot= np.random.uniform(100.,500.)
    Tnt= np.random.uniform(100.,150.)
...
ydt.to_csv('Mzvsgp3-'+sys.argv[1]+'.csv')

t2= time.time()
print ("The program spent", t2-t1, "s running:",Num,"times -> Mzvsgp3-"+sys.argv[1]+".csv")
